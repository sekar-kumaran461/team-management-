...
if pip install -r requirements.txt; then
    ...
    if pip install --no-build-isolation Django==4.2.7 gunicorn==20.1.0 whitenoise==6.5.0 python-decouple==3.8 dj-database-url==2.1.0 django-allauth==0.54.0 djangorestframework==3.14.0 django-cors-headers==4.3.1 Pillow==9.5.0; then
        ...
        if pip install --no-build-isolation Django==4.2.7 gunicorn==20.1.0 whitenoise==6.5.0; then
            echo "✅ Core packages installed, adding critical dependencies..."
            DB_TYPE="sqlite"
            PACKAGE_SET="minimal"
            
            # Install critical packages individually with verification
            echo "📦 Installing python-decouple..."
            pip install python-decouple==3.8 || echo "⚠️  python-decouple failed"
            
            echo "📦 Installing django-allauth (critical for middleware)..."
            if pip install django-allauth==0.54.0; then
                echo "✅ django-allauth installed successfully"
            else
                echo "❌ django-allauth failed - trying without version pin..."
                pip install django-allauth || echo "❌ django-allauth completely failed"
            fi
            ...
            # STRATEGY 4: Try minimal requirements file
            echo "🔄 Strategy 4a: Attempting minimal requirements file..."
            if pip install -r requirements-minimal.txt; then
                echo "✅ Minimal requirements file installed"
                DB_TYPE="sqlite"
                PACKAGE_SET="emergency-minimal"
            else
                echo "🔄 Strategy 4b: Emergency bare minimum installation..."
...
for pkg, name in packages:
    try:
        ...
        if pkg in ['django', 'allauth']:  # Critical packages
            missing_critical.append(name)

if missing_critical:
    print(f'❌ Critical packages missing: {missing_critical}')
    print('🔄 Attempting to install missing critical packages...')
    for pkg_name in missing_critical:
        if pkg_name == 'django-allauth':
            import subprocess
            try:
                subprocess.run([sys.executable, '-m', 'pip', 'install', 'django-allauth==0.54.0'], check=True)
                print('✅ django-allauth installed via subprocess')
            except:
                try:
                    subprocess.run([sys.executable, '-m', 'pip', 'install', 'django-allauth'], check=True)
                    print('✅ django-allauth installed (no version)')
                except:
                    print('❌ django-allauth installation failed completely')
"
